		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, p := range args {
				...
				if err != nil && !errors.Is(err, os.ErrNotExist) {
					// Couldn't resolve via PATH; we still try to match the
					// argument against the config directly below.
					log.Debugf("could not resolve %q via PATH: %v", p, err)
				}

				// Find the managed binary this argument refers to, matching by
				// resolved path, stored path, expanded path or base name.
			...
			if len(removed) == 0 {
				log.Warn("No binaries were removed")
			} else {
				log.Infof("Done, removed %d binary(s)", len(removed))
			}
			return nil
		},
	}
